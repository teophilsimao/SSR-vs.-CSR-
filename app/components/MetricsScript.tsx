        ...
        setTimeout(async () => {
          ...
          const metrics = {
            ...
            deviceInfo: {
              ...
              deviceMemory: 'memory' in navigator ? (navigator as any).deviceMemory : undefined,
      ...
      const interactionObserver = new PerformanceObserver((entryList) => {
        ...
        if (entries.length > 0) {
          entries.forEach((entry: any) => {
            // Store all INP entries for percentile calculation
            if (window.__PERFORMANCE_METRICS__?.inpEntries) {
              window.__PERFORMANCE_METRICS__.inpEntries.push(entry.duration);
            }
            // Also store the latest value for backwards compatibility
            if (window.__PERFORMANCE_METRICS__) {
              window.__PERFORMANCE_METRICS__.inp = entry.duration;
            }
          });
        }
      });
